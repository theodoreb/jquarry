...
function getMethodUse(tokens, API) {
  ...
  var excludeListIdentifiers = ['behaviors', 'this', 'self', 'tab_focus'];
  ...
  tokens.forEach(function (token) {
    ...
    // Ajax is special. We don't track transport types so it's best guess.
    if (token.type === 'String') {
      var ajaxDep = false;
      switch (token.value.replace(/'/g, '').replace(/"/g, '')) {
        case 'jsonp':
          ajaxDep = {method: 'jsonp', object: 'jQuery.ajax', module: 'ajax/jsonp'};
          break;

        case 'json':
          ajaxDep = {method: 'json', object: 'jQuery.ajax', module: 'ajax/parseJSON'};
          break;

        case 'xml':
          ajaxDep = {method: 'xml', object: 'jQuery.ajax', module: 'ajax/parseXML'};
          break;

        case 'script':
          ajaxDep = {method: 'script', object: 'jQuery.ajax', module: 'ajax/script'};
          break;
      }

      if (ajaxDep) {
        used.push(ajaxDep);
        used.push({method: 'xhr', object: 'jQuery.ajax', module: 'ajax/xhr'});
      }
    }
  });
...
module.exports = function (content) {
  ...
  var allModules = _.keys(JQModules);
  // Check whether Sizzle selectors are loaded.
  if (allModules.indexOf('sizzle') === -1) {
    if (!(/Selectors?/g).test(allModules.join(','))) {
      JQModules['selector-native'] = [];
    }
    else {
      JQModules['sizzle'] = [];
    }
    ...
    JQModules['core/parseHTML'] = ['sizzle'];
  }
